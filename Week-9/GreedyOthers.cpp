...
{
    cout << "********************** Minimizing Waiting Time ************************\n" << endl;

    int n;
    cout << "Enter number of customers : ";
    cin >> n;

    myPair<int,int> *customerInfo = new myPair<int,int>[n];

    cout << "Enter service times : \n\t";
    for (int i = 0; i < n; ++i) {
        customerInfo[i].first = (i+1);
        cin >> customerInfo[i].second;
    }

    myQuickSort(customerInfo, 0, (n-1));

    cout << "Order of service : \n\t";
    for (int k = 0; k < n; ++k) {
        cout << customerInfo[k].first << " ";
    }
    cout << endl;
